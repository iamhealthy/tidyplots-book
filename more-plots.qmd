...
## Dumbbell plot with data labels

```{r}
library(tidyplots)

# Calculation
study_fct <- study |> # ensure the same order for `participant` and its levels
  dplyr::arrange(group, rev(participant)) |> 
  dplyr::mutate(participant = forcats::fct_inorder(participant))
...
#| eval: false
# Plot
p4 <- study_fct |> # from last section but with reversed x levels
  tidyplot(
    x = participant, 
    y = score, 
    color = group, 
...
p5 <- p4 |> 
  add_title(title = "p5: via 'add_annotation_text()'\n'placebo' labels") |> 
  add_annotation_text(
    text = study_fct$score[study_fct$group == "placebo"], 
    x = 1:10, # see the note of next page
    y = study_fct$score[study_fct$group == "placebo"], 
    vjust = 2) |> # tailored value
  remove_clipping() |> 
  adjust_padding(bottom = 0.1) # to accommodate the labels for the "placebo" group

p6 <- p5 |> 
  add_title(title = "p6: via 'add_annotation_text()'\n'treatment' labels") |> 
  add_annotation_text(
    text = study_fct$score[study_fct$group == "treatment"], 
    x = 1:10, # see note
    y = study_fct$score[study_fct$group == "treatment"],
    vjust = -1) # tailored value
```

```{r}
#| eval: false
#| echo: false
patchwork::wrap_plots(p4, p5, p6, ncol = 1) |> 
  save_plot("images/dumbbell_data-labels.png",
    view_plot = FALSE, width = 85, height = 210)
```

![Dumbbell plot with data labels.](images/dumbbell_data-labels.png){fig-align="center" width="50%"}

:::{.callout-note icon="true"}
> Internally, `ggplot2` (and of course `tidyplots`) handles discrete scales by mapping each category to an integer value and then drawing the geom at the corresponding coordinate location. <https://ggplot2-book.org/scales-position.html#sec-discrete-position>
:::
...
![Pie and donut plots.](images/donut-pie.png){width="100%" fig-align="center"}
...
p5 <- climate |> 
  ...
  add_histogram(
    alpha = 0.5, 
    position = "identity") # "stack" in default

p6 <- climate |> 
  dplyr::filter(month %in% c("01", "03", "06")) |> 
  tidyplot(x = max_temperature, color = month) |> 
  add_title(title = "p6: density_smooth") |> 
  add(ggplot2::geom_density(alpha = 0.2))
```
