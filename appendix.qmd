...
df |> tidyplot(
    x = x, y = y) |> 
  add_data_points(
    shape = df$shape, size = 3, 
    color = "#000000", fill = "#bb5566") |> 
  add_annotation_text(
    text = df$shape, 
    x = df$x, y = df$y, 
    vjust = 2.5, 
...
## `Patchwork` for combining `tidyplots` and `tidyheatmaps` plots

:::{.callout-note icon="true"}
> The goal of `tidyheatmaps` <https://github.com/jbengler/tidyheatmaps/> is to simplify the generation of publication-ready heatmaps from tidy data.
:::
...
gene_expression |> 
  dplyr::select(external_gene_name, sample, expression) |> 
  dplyr::slice_head(n = 5)
```
...
p3 <- gene_expression |>  
  tidyheatmap(
    ...
    columns = sample,
    values = expression,
    colors = c("#f7f8f9", "#004488"),
    cluster_rows = TRUE,
    silent = TRUE) |> 
  ggplotify::as.ggplot() + # to 'ggplot' object for 'ggplot2' ecosystem
  ggplot2::theme(
    panel.widths = ggplot2::unit(120, "mm"))

design <- "AC
           BC"

(patchwork::wrap_plots(p1, p2, p3, design = design) +
  patchwork::plot_annotation(tag_levels = "A") &
  ggplot2::theme(plot.tag = ggplot2::element_text(size = 12, face = "bold"))) |> 
  save_plot("images/patchwork_combine_tidyheatmap.png",
    view_plot = FALSE, width = 205, height = 160) #width and height should be tailored
```

![`Patchwork` for combining `tidyplots` and `tidyheatmaps` plots.](images/patchwork_combine_tidyheatmap.png){fig-align="center" width="100%"}
...
### To cite this cookbook in publications use: {.unlisted .unnumbered}

```{r}
#| eval: false
#| code-overflow: wrap
Wu K. Tidyplots: publication-ready plots for scientific papers: A cookbook. 2026. Available at: https://tidyplots-book.pages.dev/
```

### A BibTeX entry for LaTeX users is {.unlisted .unnumbered}

```{r}
#| eval: false
#| code-overflow: wrap
@Book{wk2026tidyplots,
  author = {Wu, Kang},
  title = {Tidyplots: publication-ready plots for scientific papers: A cookbook},
  year = {2026},
  url = {https://tidyplots-book.pages.dev/}
}
```