# Plot types and techniques (part 1)
...
## Legend position (e.g. areastack plot)
...
p2 <- p1 |> 
  ...
  adjust_legend_position(
    position = "bottom") # or "top", "left"

p3 <- p2 |> 
  add_title(
    title = "p3: adjust legend dimension") |> 
  adjust_colors(
...
p2 <- p1 |> 
  adjust_title(
    title = "p2: 'bottom'\ntake up too much width") |> 
  adjust_legend_position(position = "bottom") # or "top", "left"
...
#| echo: false
patchwork::wrap_plots(p1, p2, p3, ncol = 1) |> 
  save_plot("images/legend-position.png",
    view_plot = FALSE, width = 185, height = 280)
```

![Adjust legend position.](images/legend-position.png){fig-align="center" width="100%"}
...
p1 <- study |> 
  tidyplot(
    ...
    color = dose) |> 
  add_mean_bar() |> 
  add_title(title = "p1: default style")
...
p2 <- study |> 
  tidyplot(
    ...
    color = dose) |> 
  add_mean_bar() |> 
  add_title(title = "p2: defined global style")
```
...
p2 <- study |> 
  tidyplot(
    ...
    color = dose) |> 
  add_mean_bar() |> 
  add_title(title = "p2: defined custom style") |> 
  my_style()
```
...
## Set a custom style to reuse (e.g. areastack plot)
...
p2 <- p1 |> 
  ...
  adjust_legend_title(title = "ENERGY_TYPE", color = "#bb5566") |>   
...
patchwork::wrap_plots(p1, p2, p3, ncol = 1) |> 
  save_plot("images/set-custom-style_areastack.png",
    view_plot = FALSE, width = 120, height = 210)
```

![Set a custom style to reuse.](images/set-custom-style_areastack.png){width=90% fig-align="center"}
