...
p2 <- study_c |> 
  tidyplot(x = group, y = mean, color = dose) |> 
  add_barstack_absolute(reverse = TRUE, alpha = 0.5) |> 
  add_title(title = "p2: absolute barstack\nreverse")

p3 <- p2 |> 
  adjust_title(title = "p3: vertical lines") |> 
  add_annotation_line(
    x = study_c$gpos, xend = study_c$gpos,
    y = study_c$lower, yend = study_c$upper)

p4 <- p3 |> 
  adjust_title(title = "p4: upper lines") |> 
  add_annotation_line(
    x = study_c$gpos - 0.2, xend = study_c$gpos + 0.2,
    y = study_c$upper, yend = study_c$upper)

p5 <- p4 |> 
  adjust_title(title = "p5: lower lines") |> 
  add_annotation_line(
    x = study_c$gpos - 0.2, xend = study_c$gpos + 0.2,
    y = study_c$lower, yend = study_c$lower)

p6 <- p2 |> 
  adjust_title(title = "p6: via ggplot2") |> 
  add(ggplot2::geom_errorbar(
    ggplot2::aes(ymin = lower, ymax = upper), 
    width = 0.4, show.legend = FALSE, color = "#000000"))
```
